...
const char* getInlineTypeRead (const VariableBinStruct& vbs)
{
	...
	case VariableBinStruct::TYPE_INT_S32 :
		return "ba.readInt();";
	case VariableBinStruct::TYPE_INT_S8_16 :
		return "ByteArrayUtil.readByteOrShort(ba);";
	case VariableBinStruct::TYPE_INT_S8_16_32 :
		return "ByteArrayUtil.readByteOrShortOrInt(ba);";
	case VariableBinStruct::TYPE_INT_S16_32 :
		return "ByteArrayUtil.readShorteOrInt(ba);";
	...
	case VariableBinStruct::TYPE_UINT_U8_16 :
		return "ByteArrayUtil.readUnsignedByteOrShort(ba);";
	case VariableBinStruct::TYPE_UINT_U8_16_32 :
		return "ByteArrayUtil.readUnsignedByteOrShortOrInt(ba);";
	case VariableBinStruct::TYPE_UINT_U16_32 :
		return "ByteArrayUtil.readUnsignedShorteOrInt(ba);";
	case VariableBinStruct::TYPE_NUMBER_F32 :
		return "ba.readFloat();";
	case VariableBinStruct::TYPE_NUMBER_D64 :
		return "ba.readDouble();";
	case VariableBinStruct::TYPE_STRING_ID :
		return  "a_env.getString(ByteArrayUtil.readUnsignedByteOrShort(ba));";
	case VariableBinStruct::TYPE_INSTANCE_UID :
		if (vbs.inlineClassId != -1 && (vbs.type == VariableBinStruct::TYPE_INSTANCE_UID))
		{	
...
{	
	...
	if (argc == 1 ||  
		...
		(argc == 3 && argv[1][0] == '-' && (argv[1][1] == 'p' || argv[1][1] == 'P'))
			...
			if (readFileHead(ba))
			{
				...
				for (int classi = 0; classi < s_classPool_index ; classi++)
				{
					...
					{	
						...
						output.writeMultiByte(" extends ExbnClassBase {\n");

						output.writeMultiByte("\n\t\tpublic function ");
						output.writeMultiByte(_cbs.className);
						output.writeMultiByte("(a_instanceUID : uint)\n\t\t{\n\t\t\tsuper(a_instanceUID);\n\t\t}\n");
						...
						for (int vi = 0 ; vi <  _cbs.variableNum; vi++ )
						{
							...
							output.writeMultiByte("\t\tpublic var ");
						...
						output.writeMultiByte("\n\n");
						{
							...
							output.writeMultiByte("\t\tpublic override function decode(ba : ByteArray , a_env : ExbnEnv) : void {\n");
						...
						sprintf_s(outputfilename, 256, "%s.as" , _cbs.className);
	...
	{
		_tprintf(_T("\a"));//di
		_tprintf( _T("plz use as ExbnExporter inputFilename (-package classpackage)  \ne.g. ExbnExporter c:\\a.exbn -package com.qq.angel\n"));
